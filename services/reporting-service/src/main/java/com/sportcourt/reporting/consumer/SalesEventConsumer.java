...
import com.sportcourt.reporting.service.SalesProjectionService;
...
public class SalesEventConsumer {
    ...
    public SalesEventConsumer(ObjectMapper objectMapper,
                              ProjectionEventService projectionEventService,
                              SalesProjectionService salesProjectionService) {
        this.objectMapper = objectMapper;
        this.projectionEventService = projectionEventService;
        this.salesProjectionService = salesProjectionService;
    }

    @KafkaListener(
        topics = "${kafka.topics.sales-events}",
        groupId = "${kafka.consumer.sales.group-id}",
        autoStartup = "${kafka.consumer.sales.enabled:true}"
    )
    public void consume(@Payload String payload,
        ...
        try {
            ...
            if (!"SALES_ORDER_CREATED".equals(type)) {
                return;
            }

            UUID orderId = parseUuid(event.path("orderId").asText(null));
            if (orderId == null) {
                return;
            }
            salesProjectionService.projectOrderCreated(
                orderId,
                parseUuid(event.path("bookingId").asText(null)),
                parseUuid(event.path("venueId").asText(null)),
                parseUuid(event.path("customerId").asText(null)),
                parseBigDecimal(event.path("totalAmount").asText(null)),
            ...
            throw new IllegalStateException("Failed to project sales event", ex);
        }
    }
    ...
    private void validateSchemaVersion(String schemaVersion) {
        ...
        if (!SUPPORTED_SCHEMA_VERSION.equals(schemaVersion)) {
            throw new IllegalArgumentException("Unsupported sales event schemaVersion: " + schemaVersion);
        }
    }
