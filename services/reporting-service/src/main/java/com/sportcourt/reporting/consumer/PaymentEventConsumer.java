...
public class PaymentEventConsumer {
    ...
    @KafkaListener(
        ...
        groupId = "${kafka.consumer.payment.group-id}",
    ...
    public void consume(@Payload String payload,
        ...
        try {
            ...
            if (!"DEPOSIT_SUCCEEDED".equals(type)) {
                return;
            }

            bookingProjectionService.projectDeposit(
                bookingId,
                parseBigDecimal(event.path("amount").asText(null)),
                parseOffsetDateTime(event.path("occurredAt").asText(null))
            );
        } catch (IllegalArgumentException ex) {
            throw ex;
        } catch (Exception ex) {
            throw new IllegalStateException("Failed to project payment event", ex);
        }
    }
