...
## SportCourt adaptation (mapped with checklist)

This document is a reference style system. For SportCourt, apply only UI patterns that do not conflict with the existing frontend checklist and booking funnel rules.

### Applied now
- Keep booking/payment flow intact (no extra steps, no route changes).
- Keep focus mode in booking/payment (bottom nav hidden in flow screens).
- Move visual language toward:
  - blue CTA pills,
  - white/soft-gray surfaces,
  - larger card radius,
  - cleaner shadows,
  - clearer visual hierarchy for search/listing/action.

### Explicitly skipped due checklist conflicts
- Any pattern that increases booking friction (extra mandatory interactions).
- Any e-commerce-only behavior unrelated to sports booking domain.
- Any redesign that breaks current route/feature parity required by frontend phases.
